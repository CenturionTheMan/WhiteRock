...
FOREX_PAIRS = {
    "EURUSD=X": {"name": "EUR/USD", "pip": 0.0001, "spread_pips": 1.0,  "group": "Major"},
    "GBPUSD=X": {"name": "GBP/USD", "pip": 0.0001, "spread_pips": 1.5,  "group": "Major"},
    "USDJPY=X": {"name": "USD/JPY", "pip": 0.01,   "spread_pips": 1.0,  "group": "Major"},
    "USDCHF=X": {"name": "USD/CHF", "pip": 0.0001, "spread_pips": 1.5,  "group": "Major"},
    "AUDUSD=X": {"name": "AUD/USD", "pip": 0.0001, "spread_pips": 1.5,  "group": "Major"},
    "USDCAD=X": {"name": "USD/CAD", "pip": 0.0001, "spread_pips": 1.5,  "group": "Major"},
    "NZDUSD=X": {"name": "NZD/USD", "pip": 0.0001, "spread_pips": 2.0,  "group": "Major"},
    "EURGBP=X": {"name": "EUR/GBP", "pip": 0.0001, "spread_pips": 2.0,  "group": "Cross"},
    "EURJPY=X": {"name": "EUR/JPY", "pip": 0.01,   "spread_pips": 2.0,  "group": "Cross"},
    "GBPJPY=X": {"name": "GBP/JPY", "pip": 0.01,   "spread_pips": 3.0,  "group": "Cross"},
}
...
def load_forex(ticker: str, start: str = "2020-01-01", end=None) -> pd.DataFrame:
    try:
...
def adx_indicator(high: pd.Series, low: pd.Series, close: pd.Series, period=14):
...
def make_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    bb_u, bb_l, bb_w = bollinger(c)
    out["bb_upper"] = bb_u
    out["bb_lower"] = bb_l
    out["bb_width"] = bb_w
    out["bb_pos"]   = (c - bb_l) / ((bb_u - bb_l) + 1e-12)

    k_pct, d_pct = stochastic(h, l, c)
    out["stoch_k"]  = k_pct
    out["stoch_d"]  = d_pct
    ...
    adx_v, pdi, mdi = adx_indicator(h, l, c, 14)
    out["adx"]      = adx_v
    out["plus_di"]  = pdi
    out["minus_di"] = mdi
    out["di_diff"]  = pdi - mdi
    ...
    for w in [5, 10, 20]:
        out[f"pct_from_high_{w}"] = (c - h.rolling(w).max()) / (h.rolling(w).max() + 1e-12)
        out[f"pct_from_low_{w}"]  = (c - l.rolling(w).min())  / (l.rolling(w).min()  + 1e-12)

    out = out.replace([np.inf, -np.inf], np.nan)
    return out

def make_direction_label(df_feat: pd.DataFrame, horizon: int = 5, thr: float = 0.003):
...
class ForexConfig:
    ...
    test_months:    int   = 3
    horizon:        int   = 5
    direction_thr:  float = 0.003   # ~30 pips EURUSD; 0.3%
    conf_threshold: float = 0.45    # min prob to enter
    spread_pips:    float = 1.0     # overridden per pair
...
def walk_forward_multi_model(df: pd.DataFrame, cfg: ForexConfig):
    from sklearn.preprocessing import StandardScaler

    feat          = make_features(df)
    y_dir, fwd    = make_direction_label(feat, horizon=cfg.horizon, thr=cfg.direction_thr)
    X_cols        = [c for c in feat.columns if c not in _DROP_FROM_X]
    data          = pd.concat([df[["Close", "High", "Low"]], feat[X_cols], y_dir.rename("y")], axis=1).dropna()
...
def backtest_forex(df: pd.DataFrame, signal: pd.Series, cfg: ForexConfig, feat: pd.DataFrame):
    idx     = df.index
    ...
    for i in range(1, len(idx)):
        ...
        if in_pos == 0 and new_sig != 0:
            ...
            a           = float(atr14.iloc[i]) if not np.isnan(atr14.iloc[i]) else c.iloc[i] * 0.005
            if cfg.use_sl_tp:
...
def get_feature_importance(df: pd.DataFrame, cfg: ForexConfig) -> Optional[pd.Series]:
...
def plot_pair_dashboard(
    ...
    cfg: ForexConfig, out_dir: str,
):
    name = FOREX_PAIRS.get(ticker, {}).get("name", ticker)
    c    = df["Close"].squeeze()

    fig  = plt.figure(figsize=(20, 16))
    fig.suptitle(f"Forex ML Dashboard — {name}", fontsize=14, fontweight="bold", y=0.99)
    ...
    ens_sig = signals.get("Ensemble", pd.Series(0, index=c.index, dtype=int)).reindex(c.index).fillna(0).astype(int)
    prev    = ens_sig.shift(1).fillna(0).astype(int)
    long_entry  = (prev == 0) & (ens_sig ==  1)
    short_entry = (prev == 0) & (ens_sig == -1)
    any_exit    = (prev != 0) & (ens_sig ==  0)

    ax_price.scatter(c.index[long_entry],  c[long_entry],  marker="^", s=55,
                     color="#00C853", zorder=5, label="Long entry")
    ax_price.scatter(c.index[short_entry], c[short_entry], marker="v", s=55,
                     color="#D50000", zorder=5, label="Short entry")
    ax_price.scatter(c.index[any_exit],    c[any_exit],    marker="o", s=30,
                     color="#FF6F00", zorder=5, alpha=0.7, label="Exit")
    ...
    if "macd_hist" in feat.columns:
        ...
        bar_colors = ["#43A047" if v >= 0 else "#E53935" for v in mh.values]
        ax_macd.bar(mh.index, mh.values, color=bar_colors, alpha=0.55, width=1.2)
    ...
    for mname, eq_s in equities.items():
        ...
        label = f"{mname} (Sh={sh:.2f}, CAGR={cagr*100:.1f}%)"
...
def plot_feature_importance(ticker: str, fi: pd.Series, out_dir: str, top_n: int = 25):
    ...
    name = FOREX_PAIRS.get(ticker, {}).get("name", ticker)
    ax.set_title(f"Top {top_n} Features — {name}", fontsize=11)
    fig.tight_layout()
...
def plot_model_comparison_bar(summary_df: pd.DataFrame, out_dir: str):
    """Sharpe / CAGR / MaxDD per model across all pairs."""
    metrics = ["Sharpe", "CAGR", "MaxDD"]
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    fig.suptitle("Model Performance Comparison (average across pairs)", fontsize=12, fontweight="bold")
...
def plot_all_equity_curves(results: Dict, out_dir: str):
    ...
    fig.suptitle("All Forex Pairs — Equity Curves by Model", fontsize=13, fontweight="bold")
    ...
    for idx, ticker in enumerate(pairs):
        ...
        ax.set_title(FOREX_PAIRS.get(ticker, {}).get("name", ticker), fontsize=10)
    ...
    fig.tight_layout()
    path = os.path.join(out_dir, "plots", "all_pairs_equity.png")
...
def plot_summary_scatter(summary_df: pd.DataFrame, out_dir: str):
    """Sharpe vs MaxDD scatter, one dot per (pair, model)."""
    ens = summary_df[summary_df["Model"] == "Ensemble"].copy()
    if ens.empty:
        return

    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    fig.suptitle("Forex Ensemble — Cross-Pair Summary", fontsize=12, fontweight="bold")

    ax1, ax2 = axes

    pairs_labels = [FOREX_PAIRS.get(p, {}).get("name", p) for p in ens["Pair"]]
    sh_vals = ens["Sharpe"].fillna(0).values
    colors = ["#43A047" if v > 0 else "#E53935" for v in sh_vals]
    x = np.arange(len(pairs_labels))
    ax1.bar(x, sh_vals, color=colors, edgecolor="white")
    ax1.set_xticks(x)
    ax1.set_xticklabels(pairs_labels, rotation=35, ha="right", fontsize=8)
    ax1.axhline(0, color="black", lw=0.6)
    ax1.set_ylabel("Sharpe Ratio")
    ax1.set_title("Sharpe Ratio per Pair (Ensemble)", fontsize=10)

    cagr_vals = ens["CAGR"].fillna(0).values * 100
    maxdd_vals = ens["MaxDD"].fillna(0).values * 100
    sc = ax2.scatter(np.abs(maxdd_vals), sh_vals, s=90,
                     c=cagr_vals, cmap="RdYlGn", edgecolors="black", linewidths=0.6, zorder=3)
    plt.colorbar(sc, ax=ax2, label="CAGR (%)")
    for i, lbl in enumerate(pairs_labels):
        ax2.annotate(lbl, (abs(maxdd_vals[i]), sh_vals[i]),
...
def run_forex(
    ...
    cfg: Optional[ForexConfig]   = None,
    out_dir: str                 = "outputs_forex_ml",
):
    if cfg     is None: cfg     = ForexConfig()
    ...
    for ticker in tickers:
        ...
        for mname in model_names:
            ...
            try:
                s_ret, eq, pos = backtest_forex(df, sig, pair_cfg, feat)
                st = perf_stats(s_ret)
                equities[mname]  = eq
                stats_all[mname] = st
                summary_rows.append({
                    "Pair": ticker, "Name": name, "Model": mname,
                    **st,
        ...
        try:
            ...
            print(f"  [WARN] Monthly heatmap: {e}")
    ...
    print(f"\nOutputy w: {out_dir}/")
    print(f"  plots/          — wykresy PNG")
    print(f"  signals/        — sygnały CSV per para")
    print(f"  *_walkforward.csv — okna walk-forward")
...
if __name__ == "__main__":
    ...
    run_forex(tickers=tickers, cfg=cfg, out_dir="outputs_forex_ml")
